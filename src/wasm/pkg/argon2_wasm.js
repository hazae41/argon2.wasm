...
export class Argon2Deriver {
    ...
    [Symbol.dispose]() {
        ...
        wasm.__wbg_argon2deriver_free(ptr, 0);
    }
    /**
     ...
     */
    constructor(algorithm, version, memory, iterations, parallelism) {
        const ptr0 = passStringToWasm0(algorithm, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.argon2deriver_new(ptr0, len0, version, memory, iterations, parallelism);
        if (ret[2]) {
    ...
    /**
     * @param {Memory} password
     * @param {Memory} salt
     * @returns {Memory}
     */
    derive(password, salt) {
        _assertClass(password, Memory);
        _assertClass(salt, Memory);
        const ret = wasm.argon2deriver_derive(this.__wbg_ptr, password.__wbg_ptr, salt.__wbg_ptr);
        if (ret[2]) {
            throw takeFromExternrefTable0(ret[1]);
        }
